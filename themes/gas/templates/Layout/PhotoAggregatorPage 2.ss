...
<div class="row layout">
	...
	<div class="large-12 columns">
		...
		<% if Photos %>
			<ul class="large-block-grid-3 small-block-grid-2 blocks results">
				<% loop Photos %>
				<li class="item">
					...
					\$(".blocks .photo").magnificPopup({
						...
						image: {
							titleSrc: function(item) {
							  ...
							  return '  <a href="' + item.el.attr("data-projectlink") + '" title="View entire photo gallery" class="projectlink">View entire gallery of <strong>' + item.el.attr("data-projecttitle") + '</strong> <i class=" icon-angle-circled-right"></i></a> '+
				    ...
				    \$.ajax(
				    {
				        ...
				        success:function(data, textStatus, jqXHR) 
				        {
				            if(data!="") {
				            	...
				            	if(page == 0) {
				            		\$container.empty().html(data).masonry('reload');
				            	}
				            	else {
