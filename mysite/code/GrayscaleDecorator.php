...
public function generateGreyscaleImage(GDBackendGD $gd, $RGB)
{
    $Vars = explode(' ', $RGB);
    return $gd->greyscale( $Vars[0], $Vars[1], $Vars[2]);
}

public function generateScaledGreyscaleImage(GDBackend $gd, $width, $RGB)
{
    $Vars = explode(' ', $RGB);
...
public function generateGreyPaddedImage(GDBackend $gd, $width, $height) {
  return $gd->paddedResize($width, $height, '222222');
}

public function WhitePaddedImage($width, $height) {
	return $this->owner->getFormattedImage('WhitePaddedImage', $width, $height);
}

public function generateWhitePaddedImage(GDBackend $gd, $width, $height) {
  return $gd->paddedResize($width, $height, 'FFFFFF');
}
